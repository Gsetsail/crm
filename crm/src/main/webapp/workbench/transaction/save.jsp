	...
	<script type="text/javascript">
		 ...
		 $(function (){
			...
			$("#activityBtn").click(function(){
				...
				$("#likeActivity").keydown(function(event){
					if(event.keyCode == 13){
						if($("#likeActivity").val() == ""){
							alert("请输入市场活动的名称")
						}else{
							var name = $("#likeActivity").val();
							$.ajax({
								...
								success : function(data){
									...
									$.each(data,function(index,jsonObj){
										html += '<tr>'
										html += '<td><input type="radio" name="xz" value="'+jsonObj.id+'"/></td>'
										html += '<td id="'+jsonObj.id+'">'+jsonObj.name+'</td>'
				 ...
				 $("#contactsBtn").click(function(){
					 ...
					 $("#likeContacts").keydown(function(event){
						 if(event.keyCode == 13){

							 if($("#likeContacts").val()==""){
								 alert("请输入联系人名称")
							 }else{
								var fullname =  $("#likeContacts").val();
								$.ajax({
									...
									success : function(data){
										...
										$.each(data,function(index,jsonObj){
											html += '<tr>'
											html += '<td><input type="radio" name="xx" value="'+jsonObj.id+'"/></td>'
											html += '<td id="'+jsonObj.id+'">'+jsonObj.fullname+'</td>'
											html += '<td>'+jsonObj.email+'</td>'
											html += '<td>'+jsonObj.mphone+'</td>'
											html += '</tr>'
										})
					...
					$("#submitContactsBtn").click(function(){
							var id = $("input[name=xx]:checked").val();
							$("#contactsId").val(id)
			 	...
			 	$("#contactsMsgBtn").click(function (){
					$("#customerName").val($("#create-customerName").val());
					 $("#submitContactsForm").submit();
				})
			 ...
			 $.ajax({
				 ...
				 success : function(data){
					// [{用户1},{用户2}...]
					 ...
					 var name = "${user.id}"
					 $("#create-transactionOwner").val(name)
				 }
			 })
		 }
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" class="form-control" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询" id="likeActivity">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" class="form-control" style="width: 300px;" placeholder="请输入联系人名称，支持模糊查询" id="likeContacts">
						    <span class="glyphicon glyphicon-search form-control-feedback"></span>
						  </div>
						</form>
					</div>
	...
	<div style="position:  relative; left: 30px;">
		<h3>创建交易</h3>
	  	<div style="position: relative; top: -40px; left: 70%;">
			<button type="button" class="btn btn-primary" id="contactsMsgBtn">保存</button>
			<button type="button" class="btn btn-default">取消</button>
		</div>
		<hr style="position: relative; top: -40px;">
	</div>
	<form class="form-horizontal" role="form" style="position: relative; top: -30px;" id="submitContactsForm" method="post" action="workbench/tran/insertTran.do">
		...
		<div class="form-group">
			...
			<label for="create-activitySrc" class="col-sm-2 control-label">市场活动源&nbsp;&nbsp;<a href="javascript:void(0);" id="activityBtn" ><span class="glyphicon glyphicon-search"></span></a></label>
			<div class="col-sm-10" style="width: 300px;">
